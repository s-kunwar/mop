...
def clean_experience_file(df):
    ...
    # For experience files, headers are typically on row 7 (index 6)
    # Check if we have enough rows and if row 7 contains the actual headers
    if len(df) > 6:
        # Check if row 6 (0-indexed) contains the actual headers
        header_row = df.iloc[6]  # This should be the header row (7th row)
        # Check if this row contains typical header names
        if any(col in ['Name', 'USN', 'Aptitude Test', 'JAM', 'Interview', 'Email', 'Contact Number', 'CGPA', 'Company Name', 'Package'] for col in header_row.astype(str)):
            # Remove the first 7 rows (institutional information)
